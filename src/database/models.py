...
class User(Base):
    ...
    created: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.now(pytz.timezone(settings.TIMEZONE)), doc="Дата и время создания записи")
    ...
    last_action: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(pytz.timezone(settings.TIMEZONE)), doc="Дата и время последнего действия пользователя")